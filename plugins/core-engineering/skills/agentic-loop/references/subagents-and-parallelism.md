...
Default to one writer. Use independent review where required or valuable; delegate
only authorized independent work whose benefit exceeds coordination and context cost.
...
When independent review is required, requested, or justified by material risk:
...
2. **Checker** (a fresh context — the `reviewer` / `security-reviewer` agent, or a
   ...
   inspects existing validation evidence, runs additional checks only to resolve gaps,
   reads the diff against repo conventions, and reports a verdict (SAFE TO
...
The checker is adversarial by design — "reject anything not verifiably done." A same-context assessment does not satisfy an explicitly required independent review.
Reuse a completed review for the same candidate and scope across skill handoffs.
...
- **Parallel sub-agents:** when authorized, launch only within project writer limits for independent slices (e.g.
...
stash. On authorized commit, checkpoint a coherent ready batch (`committing-on-shared-trunk`).
Leave unrelated WIP intact; resolve ownership before overlapping edits.

**Explicit isolation:** when authorized, give each parallel attempt its own worktree.
Absence of the shared-tree kit does not authorize branches or worktrees. Match parallelism to review bandwidth; merge/commit/push of
...
- [ ] Required/requested review or material risk? -> independent checker on the candidate;
      reuse matching evidence instead of repeating a satisfied review.
