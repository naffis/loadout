...
description: Evidence-first root-cause debugging loop for everyday bugs. Use for a failing test, runtime error, or unexpected behavior that isn't a one-line obvious fix. For a proven class fix use root-cause-fix; for a production issue with no local repro start with debugging-with-observability.
---
...
obvious. This is the fast everyday loop; escalate to `root-cause-fix` when you need a proven,
generalized class fix, or to `debugging-with-observability` when the bug is in
...
1. **Read the actual error first — the whole thing.** Don't skim. 80% of "weird" bugs are
   ...
   assertion diff. For a type error: read the *full* error — the first line is a symptom, the
   real conflict is often several lines down. For a runtime stack: start from the frame in
   *your* code, not the dependency.
...
5. **At least two hypotheses; falsify, don't confirm.** List >=2 plausible causes ranked by
   likelihood. Add one targeted log/assert and run the repro to *disprove* your top hypothesis
